            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // 診断済みならカードも出す（今の機能維持）
                  if (diagnosed)
                    FutureBuilder<StoredDiagnosis?>(
                      future: StoredDiagnosis.load(),
                      builder: (context, snap2) {
                        final stored = snap2.data;
                        if (!snap2.hasData || stored == null) {
                          return const SizedBox.shrink();
                        }
                        return Padding(
                          padding: const EdgeInsets.only(bottom: 14),
                          child: _DangoHeroCard(stored: stored),
                        );
                      },
                    ),
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      child: Text(diagnosed ? '今日の占いへ🔮' : '診断をはじめる🍡'),
                      onPressed: () async {
                        if (!diagnosed) {
                        ...
                        if (stored == null) {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text('診断情報が見つかりませんでした。もう一度診断してください。')),
                          );
                          return;
                        }
                  ...
                  if (diagnosed) ...[
                    ...
                    OutlinedButton.icon(
                      ...
                      onPressed: () async {
                        ...
                        await Navigator.push(
                          context,
                          MaterialPageRoute(builder: (_) => StoredResultScreen(stored: stored)),
                        );
                        _refreshDiagnosed();
                      },
                    ),
